...
public class Driver {
	...
	public static void main(String[] args) {
		...
		final double outTemp = 5.0; // Outside temperature for the room
		...
		Temp.get(20).add(5.0); // Add last node with independent starting temperature

		for (int i = 0; i < 20000; ++i) { // Iterate through the time step this number of times: time = (i * timeStep)
		...
		int i = 7700; // Set position in matrix to print out temperature
		/* Iterate through last matrix entry and get the last temperature value */
		for (int j = 0; j < 21; ++j) {
			System.out.println(String.format("%.7f  " + j, Temp.get(j).get(i)));
		}
